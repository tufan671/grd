...
const pusha2 = require("discord.js");
const pusha3 = require("discord.js");

const client1 = new pusha1.Client();
const client2 = new pusha2.Client();
const client3 = new pusha3.Client();
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Arcenio Bey | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
  ...
  channel.send(`<@589079197906108416>`, {embed: fayik}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Arcenio Bey | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: " Arcenio Bey | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: " Arcenio Bey | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: ` Arcenio Bey | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Arcenio Bey | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Arcenio Bey | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: " Arcenio Bey | Rol Koruma Sistemi" }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Arcenio Bey | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Arcenio Bey | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: ` Arcenio Bey | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: " Arcenio Bey | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(member.id, { reason: ` Arcenio Bey | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: " Arcenio Bey | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "Arcenio Bey | İzinsiz Webhook Açmak!"}).catch(err => { });
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`<@589079197906108416>`, {embed: fayik}).catch(err => { })
return;
});
